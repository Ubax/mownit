...
double *calcInterpolationForRandomValues() {
    ...
    std::fstream file;
    file.open("linear.csv", std::ios_base::out | std::ios::trunc);
    file << "x,y\n";
    for(int i=0;i<nodes.size();i++){
        file << nodes[i].getX() << "," << nodes[i].getY() << "\n";
    }
    ...
    gsl_interp *interp = gsl_interp_alloc(gsl_interp_cspline, nodes.size());
    gsl_interp_init(interp, xArr, yArr, nodes.size());

    gsl_interp_accel *accel = gsl_interp_accel_alloc();

    file.open("cubic.csv", std::ios_base::out | std::ios::trunc);
    file << "x,y\n";
    for (double x = minX; x <= maxX; x += 0.1) {
